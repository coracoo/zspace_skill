...
ensure_env() {
  ...
  if [[ -z "${NAS_USER:-}" || -z "${NAS_PASSWORD:-}" || "$NAS_PASSWORD" == "你的NAS密码" ]]; then
    err "NAS_USER / NAS_PASSWORD 未设置,或还在用 zspace/.env.example 的占位符"
...
cmd_dashboard() {
  ...
  nohup "$PY" -m uvicorn dashboard.app:app --host 0.0.0.0 --port 15050 \
    >>"$log_file" 2>&1 &
  echo $! > "$pid_file"
  sleep 1.5
...
cmd_deps() {
  ...
  info "pip install -r zspace/requirements.txt…"
  "$VENV/bin/pip" install -q --upgrade pip
  "$VENV/bin/pip" install -q -r zspace/requirements.txt
  ok "依赖装好"
}
...
首次使用:
  1. cp zspace/.env.example .env && 编辑填入 NAS_PASSWORD / KEY_SSH
  2. ./start.sh deps
