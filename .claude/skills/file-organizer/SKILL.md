...
**步骤**:
1. **exec** `python .claude/skills/file-organizer/file_organizer.py audit-all --output /tmp/full-report.json`
...
```bash
# 扫所有池找重复,只看 >= 10MB 的文件
python .claude/skills/file-organizer/file_organizer.py audit-duplicates --min-size 10

# 只扫 sata14 池,限制 1000 个文件(测试)
python .claude/skills/file-organizer/file_organizer.py audit-orphans --pool sata14 --sample 1000

# 综合诊断,写 JSON
python .claude/skills/file-organizer/file_organizer.py audit-all --output /tmp/audit.json
```
