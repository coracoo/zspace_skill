...
description: 诊断 ZSpace NAS 极影视的分类与影片布局,生成整理建议报告(只读,不动 NAS)。
  触发词:极影视整理、影片库分类诊断、影视分类不规范、frds 拆分、整理影视库、影视分类合并建议、
  影片是否在对的分类、collection 错放、哪些影片分错类了。
  不适用:写操作(合并分类/移动影片/重命名)— 这些走 MCP tool + LLM 二次确认,不在这 skill 范围。
---
...
**步骤**:
1. **exec** `python .claude/skills/media-organizer/media_organizer.py audit-classifications`
2. 读输出,识别 4 类问题:
