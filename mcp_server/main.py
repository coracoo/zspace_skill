...
全局变量:
- `mcp`   — FastMCP 实例(永不重新赋值,所有 tool 文件 `from mcp_server.main import mcp` 装饰)
- `nas`   — NasClient 实例(_startup 中赋值);tool 函数通过 `from mcp_server import main as _main; _main.nas` 在调用时取最新值
...
循环 import 拓扑:
    ...
    关键:本文件里 `from mcp_server.tools import ...` 必须在 `mcp = FastMCP(...)` 之后,
...
if TYPE_CHECKING:  # pragma: no cover
    from mcp_server.zenith import ZenithSession
...
# 注意:tool 函数不能 `from mcp_server.main import nas`(那是值绑定,会停在 None);
# 必须用 `from mcp_server import main as _main; _main.nas.post(...)`(属性访问,运行时取最新值)
...
async def _startup():
    ...
    global nas, zenith
    from mcp_server.zenith import ZenithSession, ZENITH_COOKIE_EXTRA
    nas = NasClient()
    try:
        await nas.login()
