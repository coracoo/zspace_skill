            ...
            try:
                conn.execute(
                    "DELETE FROM chunks WHERE 1=1" + (
                        "" if scope == "all"
                        else f" AND source_type='{'file' if scope == 'files' else 'notebook'}'"
                    )
                )
                conn.commit()
            finally:
...
def _do_reindex(scope: str) -> int:
    ...
    同步跑(简化版,N150 限速 100ms/请求 + 进度输出到 stderr)。
...
def _reindex_files() -> int:
    """DFS 扫 NAS 池(用 MCP tool 拿,而不是 NAS API 直连 — 复用 token + 鉴权)。

    简化:直接用 NasClient 调 /v2/file/list。
    """
    import time
    from mcp_server import main as _main
    nas = _main.nas

    pools_resp = nas.post("/zspool/info", {})
    pools = pools_resp.get("data", {}).get("pools", []) if isinstance(pools_resp, dict) else []
    if not pools:
        # fallback:用 list_storage_pools MCP tool
        return _reindex_via_mcp()
    indexed = 0
    for pool in pools:
        pool_name = pool.get("name", "")
        root = f"/{pool_name}/my/data/"
        log.info("reindex files: scanning pool=%s root=%s", pool_name, root)
        indexed += _dfs_scan_and_index(nas, root)
    return indexed


def _reindex_via_mcp() -> int:
    """fallback:从 NAS 拿所有池,逐个扫。"""
    import time
    from mcp_server import main as _main
    nas = _main.nas
    indexed = 0
    ...
    while True:
        ...
        for item in items:
            if item.get("type") == "folder":
                ...
                indexed += _dfs_scan_and_index(nas, child)
                continue
...
def _reindex_notebooks() -> int:
    ...
    indexed = 0
    # 拉所有分类(顶层)
    resp = nas.post("/v2/file/notepad/classifylist", {})
    classifies = (resp.get("data") or {}).get("list", []) if isinstance(resp, dict) else []
    if not classifies:
        classifies = [{"classify_id": 0}]  # 默认根分类

    from .hooks import rag_on_notebook_write
    from .chunker import chunk_text
    from .store import delete_chunks_by_source, insert_chunks_batch

    for c in classifies:
        cid = c.get("classify_id", 0)
        list_resp = nas.post("/v2/file/notepad/list", {"classify_id": cid, "num": 200, "start": 0})
        notes = (list_resp.get("data") or {}).get("list", []) if isinstance(list_resp, dict) else []
        for note in notes:
            note_id = note.get("id")
            title = note.get("title", "")
            # 拉 body
            info = nas.post("/v2/file/notepad/info", {"id": note_id})
